...
public class evaluacion_peralta {
	...
	public static void main(String[] args) {
		...
		List<String> nombre = new ArrayList<>(); //almacena elementos de manera ordenada permitiendo modificar, eliminar,etc.
		...
		for (int i = 0; i < personas; i++) { /* Pide el ingreso de datos la cantidad de veces que el usuario asignó en el scanner anterior*/
			System.out.println("ingrese nombre: ");
			nombre.add(datos.next());/* "variable.add, ingreso de datos que permite ingresarlo directamente a la lista correspondiente.*/
		 ...
		 for (int i = 0; i< legajo.get(i); i++) { /*ordenar los datos ingresados de menor a mayor*/
			for (int j = 0; j < legajo.get(i)-1; j++) {
				if (legajo.get(i)> legajo.get(i)+1 ) {
					int templegajo; /*variable temporal, ayuda al ordenamiento */
					
					legajo.get(i); 
					
					
					System.out.println("el legajo "+ legajo.size()+"pertenece a "+ nombre+ " "+apellido);
				...
				int numingresado = datos.nextInt();
				 ...
				 if (numingresado==(legajo.get(i))) {
					 ...
					 if (encontrado == false) { /* sin la anterior condicion resultaba falsa (legajo no encontrado) mandar por pantalla el mensaje*/
